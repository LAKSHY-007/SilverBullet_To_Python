...
class SBtoPyGUI(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("SB to Python GUI Generator - By Lakshya Pendharkar")
    ...
    def parse_sb_script(self, sb_text):
        ...
        py_lines.append('        messagebox.showinfo("Credits", "Made with Love  by Lakshya Pendharkar")')
        ...
        if not requests:
            ...
            for k, v in headers.items():
                ...
                if "<UA>" in v_clean or "<User-Agent>" in v_clean or "user-agent" in k_clean.lower():
                    ...
                    py_lines.append(f'                "{k_clean}": "{v_clean}",')
            py_lines.append('            }')
            if post_data:
